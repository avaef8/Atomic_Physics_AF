...
General shielding behavior goes as follows:
- As energy increases (with electrons becoming further out), shielding constant dramatically increases. Whether the curve fits quadratically or exponentially needs to be fitted using scipy.optimize and curve_fit
- Inside each energy shell (n=1,2,3...), the shielding constant will slightly increase with an increase in angular momentum quantum number (l=0,1,2,3). The only exception to this is seen in f orbitals; only some elements with 58 or more protons (for the neutral species)
